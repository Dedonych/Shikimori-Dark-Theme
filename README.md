...
:root {
  ...
  /* Радиус рамки аватарок пользователей/клубов */
  --avatar-border: 8px;
}
```
...
```css
:root {
  /* Отключить все темы. Если использовать это, другие можно не писать. */
  --themes: none;
  /* Отключить только рождество и новый год.
  Для этой темы идут дни с 20 декабря до 3 января */
  --xmas: none;
  /* Отключить только хэллоуин. 31 октября */
  --halloween: none;
  /* Отключить только день смеха. 1 апреля */
  --fools: none;
}
```
